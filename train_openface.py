...
def main():
    ...
    if not FACE_RECOGNITION_AVAILABLE:
        print("\n[X] ERROR: face_recognition not available")
    ...
    if not os.path.exists(config.DATASET_DIR):
        print(f"\n[X] ERROR: Dataset directory not found: {config.DATASET_DIR}")
    ...
    user_dirs = [
        ...
        if os.path.isdir(os.path.join(config.DATASET_DIR, d)) and not d.startswith(".")
    ]

    if not user_dirs:
        print(f"\n[X] ERROR: No user directories found in {config.DATASET_DIR}")
    ...
    if total_images == 0:
        print("\n[X] ERROR: No images found in dataset")
        return False
    ...
    if recognizer.train(config.DATASET_DIR):
        ...
        print("=" * 60)
        print(f"\nEncodings saved to:")
        print(f"  - {config.OPENFACE_EMBEDDINGS_PATH}")
